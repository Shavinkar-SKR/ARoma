...
import { ChefHat, Clock, Package, PhoneCall, ShoppingBag, Truck } from 'lucide-react';

const steps = [
  { id: 1, name: 'Order Received', icon: Package, completed: true, current: false },
  { id: 2, name: 'Food Being Prepared', icon: ChefHat, completed: true, current: true },
  { id: 3, name: 'Ready for Pickup', icon: Truck, completed: false, current: false },
  { id: 4, name: 'Order Complete', icon: ShoppingBag, completed: false, current: false },
];
...
function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentMessageIndex((prevIndex) => (prevIndex + 1) % randomMessages.length); // Change message every 5 seconds
        ...
        <div className="max-w-4xl mx-auto">
              ...
              <div className="relative flex justify-between">
                {steps.map((step) => (
                  <div key={step.id} className="flex flex-col items-center group">
                    <div className={`relative flex h-14 w-14 items-center justify-center rounded-full transition-all duration-300 transform group-hover:scale-110 ${step.completed ? 'bg-red-600' : step.current ? 'bg-red-600 animate-pulse' : 'bg-gray-200'}`}>
                      <step.icon className={`h-7 w-7 ${step.completed || step.current ? 'text-white' : 'text-gray-500'}`} />
                    </div>
                    <p className="mt-3 text-sm font-medium text-gray-900 group-hover:text-red-600 transition-colors duration-300">{step.name}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mb-8 bg-white p-6 rounded-2xl shadow-lg transform hover:scale-[1.02] transition-all duration-300">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-xl font-medium text-gray-900">Preparation Time</h3>
                <p className="text-sm text-gray-500 mt-1">20 minutes • Our chefs are preparing your meal</p>
              </div>
            </div>
          </div>
          ...
          {currentMessage && (
            <div className="bg-white p-10 rounded-2xl shadow-lg w-full h-64 flex flex-col justify-center mb-8 mx-auto relative">
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                <h3 className="font-medium text-white text-lg text-center">{currentMessage.title}</h3>
              </div>
            </div>
          )}
        </div>
