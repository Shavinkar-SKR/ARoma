...
var initializeWebSocket = function (httpServer) {
    ...
    io.on("connection", function (socket) {
        ...
        socket.on("joinOrderRoom", function (orderId) {
            ...
            console.log("Client ".concat(socket.id, " joined room for order ").concat(orderId));
        });
        ...
        socket.on("leaveOrderRoom", function (orderId) {
            ...
            console.log("Client ".concat(socket.id, " left room for order ").concat(orderId));
        });
...
var emitOrderUpdate = function (updatedOrder) {
    if (io && isOrderWithId(updatedOrder)) { // Ensure _id is defined
        ...
        console.log("Emitted order update to room: ".concat(updatedOrder._id));
    }
    else {
